...
1. 若训练数据是线性不可分（不存在任何线性分类器可以将其正确分类），或者数据存在噪声。
 线性不可分意味着某些样本点不能满足函数间隔大于等于1的约束条件。引入松弛变量使得错分样本得到惩罚。 其他推导过程和线性可分差不多，暂时**记住下面公式**。其中$$C$$值称为**惩罚参数**，大于0，其值大时对误分类的惩罚增大，其值小时对误分类的惩罚减小，即表示使得$$\frac{1}{2}{\lVert w\Vert}^2$$尽量小，
