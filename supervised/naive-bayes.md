...
2. 在优化中的I函数是用来统计特征的。当特征为离散时，直接统计即可。但是当特征为连续值时，需要进行转换，才能统计。总结为三种模型。特征为连续值的时候假定特征符合高斯分布:[Math Processing Error]g(x,n,u)那么[Math Processing Error]

3. 贝叶斯估计：贝叶斯估计与朴素贝叶斯是不同的概念。

4. 在朴素贝叶斯法中，学习意味着估计$$P(Y=c_k)$$和P(X(j)=x(j)|Y=ck)P(X(j)=x(j)|Y=ck)。由上述分析可知，可以运用极大似然估计和贝叶斯估计来估计相应的概率。**注意其中I函数在两者相等时取1，两者不等时取0**。**记住此优化就是利用极大似然或贝叶斯来估计参数。**
