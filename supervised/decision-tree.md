...
4. 决策树的修剪：**决策树生成算法产生的决策树，会出现过拟合的现象**。因为在学习的过程中过多地考虑如何提高对训练数据的正确分类，从而构建出过于复杂的决策树。**解决这个问题的办法就是考虑决策树的复杂度，对已生成的决策树进行简化**。对已生成的决策树进行简化的过程称为剪枝（pruning）。
    ...
    决策树的损失函数可以表示为$$C_\alpha(T)=C(T)+\alpha|T|$$，**记住该公式，虽然在第三环节但是应该在第二环节的策略的**。这时C(T)表示模型对训练数据的预测误差，即模型与训练数据的拟合程度，|T|表示模型的复杂度，参数α控制二者直接的影响。较大的$$\alpha$$促使选择较简答的模型，较小反之。剪枝，就是当$$\alpha$$确定时，选择损失函数最小的模型，即损失函数最小的子树。利用损失函数最小原则进行剪枝就是用正则化的极大似然估计进行模型选择。
