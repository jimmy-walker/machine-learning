...
#Evalution
回归学习最常用的损失函数（cost function）是平方损失函数，在此情况下，回归问题可以用著名的最小二乘法来解决。把最小二乘看做是优化问题的话，那么最小二乘法和梯度下降都是求解方法的。
...
2. 最小二乘法是直接对$$\Delta$$求导找出全局最小，是非迭代法。而梯度下降法是一种迭代法，先给定一个$$\beta$$ ，然后向$$\Delta$$下降最快的方向调整$$\beta$$ ，在若干次迭代之后找到局部最小。梯度下降法的缺点是到最小点的时候收敛速度变慢，并且对初始点的选择极为敏感，其改进大多是在这两方面下功夫。
