...
4. **正则化(Regularization)**：**正则化不是只有逻辑回归存在，它是一个通用的算法和思想，所以会产生过拟合现象的算法都可以使用正则化来避免过拟合**。
    ...
    **由于模型的参数个数一般是由人为指定和调节的，所以正则化常常是用来限制模型参数值不要过大，也被称为惩罚项。一般是在目标函数(经验风险)中加上一个正则化项**，注意这里并没有指定是逻辑回归的正则化情况，给出的是一个一般情况下的正则化，根据惯例，我们不对$$\theta_0$$进行惩罚,所以最后一项中$$j$$从1开始。其中$$\lambda$$又叫做正规化参数（Regularization Parameter）。如果正规化参数太大，则会把所有参数最小化，导致近似直线，造成欠拟合。
