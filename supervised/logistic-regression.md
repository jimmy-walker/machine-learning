...
4. **正则化(Regularization)**：**正则化不是只有逻辑回归存在，它是一个通用的算法和思想，所以会产生过拟合现象的算法都可以使用正则化来避免过拟合**。
    ...
    **注意这里并没有指定是逻辑回归的正则化情况，给出的是一个一般情况下的正则化**，根据惯例，**我们不对$$\theta_0$$进行惩罚,所以最后一项中$$j$$从1开始。**其中$$\lambda$$又叫做正规化参数（Regularization Parameter）。如果正规化参数太大，则会把所有参数最小化，导致近似直线，造成欠拟合。
