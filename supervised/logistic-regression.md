...
5. 范数：向量的范数可以理解为向量的长度、或向量到零点的距离、或相应两点距离。
    ...
    因此当$$\theta_j$$大于0的时候，$$\theta_j$$会减去一个正数，导致$$\theta_j$$减小，而当$$\theta_j$$小于0的时候，$$\theta_j$$会减去一个负数，导致$$\theta_j$$又变大，因此这个正则项会导致参数$$\theta_j$$取值趋近于0，也就是为什么**L1正则能够使权重稀疏**，这样参数值就受到控制会趋近于0，得到稀疏解。
