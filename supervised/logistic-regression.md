...
可以看到，当自变量取值0时，函数值为0.5，自变量趋向于正无穷和负无穷时函数值趋近于1和0。因此我们用下式表示对一个样本$$x$$的预测，这也就是逻辑回归的模型，**记住remember此模型公式**：
...
3. **极大似然估计（最大似然估计）**：最大似然估计就是估计未知参数的一种方法，最大似然估计（Maximum Likelihood Method）是建立在各样本间相互独立且样本满足随机抽样（可代表总体分布）下的估计方法，它的核心思想是如果现有样本可以代表总体，那么**最大似然估计就是找到一组参数使得出现现有样本的可能性最大**，即从**统计学角度需要使得所有观测样本的联合概率最大化，又因为样本间是相互独立的，所以所有观测样本的联合概率可以写成各样本出现概率的连乘积**。**注意：我们常写出似然函数（各个样本出现概率的连乘积，注意在机器学习中是P(X,Y)的连乘积，不是单P(X)或P(Y)！！！！！！！！，因为样本就是x与y的对）后，取对数，求导数最大值，从而得到要求的参数。XXX的极大似然估计，就是将XXX当作参数来进行估计，所以需要将各个样本出现概率的连乘积表示为该参数的函数！**具体见naive bayes中的例子，是P(X,Y)的连乘积，以前都搞错了，除非是没有类标记，那就用P(X)的连乘积，而在逻辑回归中用了P(Y|X)，一定是某个地方简化了，还是从P(X,Y)得到的！！！！！！！！！！
...
5. 范数：向量的范数可以理解为向量的长度、或向量到零点的距离、或相应两点距离。

    **L1会趋向于产生少量的特征，而其他的特征都是0，而L2会选择更多的特征，这些特征都会接近于0。L1在特征选择时候非常有用，而L2就只是一种规则化而已**。
    
    **越小的参数说明模型越简单**：过拟合的，拟合会经过曲面的每个点，也就是说在较小的区间里面可能会有较大的曲率，这里的导数就是很大，线性模型里面的权值就是导数，所以越小的参数说明模型越简单。

    1. **L1范数**：$$\Vert x \Vert_1$$，表示为**向量中各个元素绝对值之和**
    
    当采用梯度下降方式来优化目标函数时，对目标函数进行求导，正则化项导致的梯度变化当$$\theta_j$$大于0时取1，当$$\theta_j$$小于0时取-1。（因为绝对值求导后变成1或-1）

    因此当$$\theta_j$$大于0的时候，$$\theta_j$$会减去一个正数，导致$$\theta_j$$减小，而当$$\theta_j$$小于0的时候，$$\theta_j$$会减去一个负数，导致$$\theta_j$$又变大，因此这个正则项会导致参数$$\theta_j$$取值趋近于0，也就是为什么**L1正则能够使权重稀疏**，这样参数值就受到控制会趋近于0，得到稀疏解。**L1正则还被称为 Lasso regularization。**
    2. **L2范数**：$$\Vert x \Vert_2$$，表示为**向量中各个元素的平方和然后求平方根**。
    当采用梯度下降方式来优化目标函数时，对目标函数进行求导，得到梯度变化为$$\frac{\partial \Phi(\theta)}{\partial \theta_j} = 2\theta_j$$（一般会用$$\frac{\lambda}{2}$$来把这个系数2给消掉)。

    在回归问题中，**关于L2正则的回归还被称为Ridge Regression岭回归**。
...
从而可以得到批梯度下降的参数更新公式，**记住remember此优化方法**：
