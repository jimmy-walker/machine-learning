...
2. EM算法：面对一个含有隐变量的概率模型，目标是极大化观测数据（不完全数据），即极大化$$L(θ)=logP(Y|θ)=log\sum_{z}P(Y,Z|θ)\\=log[\sum_{z}P(Z|θ)P(Y|Z,θ)]L(\theta)=logP(Y|\theta)=log\sum_{z}P(Y,Z|\theta)=log[\sum_{z}P(Z|\theta)P(Y|Z,\theta)]$$
