...
2. EM算法：面对一个含有隐变量的概率模型，目标是极大化观测数据（不完全数据），即极大化$$L(θ)=logP(Y|θ)=log∑zP(Y,Z|θ)\\=log[∑zP(Z|θ)P(Y|Z,θ)]L(\theta)=logP(Y|\theta)=log\sum_{z}P(Y,Z|\theta)=log[\sum_{z}P(Z|\theta)P(Y|Z,\theta)]$$
(\theta)    极大化的主要困难是上式中含有未观测数据log里有包含和，EM算法则是通过迭代逐步近似极大化$$L$$的。
